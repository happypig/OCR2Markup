...
A markup editor opens the plugin preferences page and sees six rows: three for the CBRD Link lookup used by Ref-to-Link (endpoint URL, Referer header, timeout) and three for the CBRD Parse endpoint used by AI Markup (endpoint URL, token, timeout). The first three rows currently read "CBRD API URL", "CBRD Referer header", and "CBRD timeout (ms)" — the "CBRD API"/"CBRD" names predate the CBRD Parse surface added by this feature, so they no longer distinguish the Ref-to-Link lookup from the AI Markup endpoint. After this change the endpoint row reads "CBRD Link Endpoint" and the timeout row reads "CBRD Link timeout (ms)", so the editor can tell which rows configure which action at a glance. Only the displayed label text changes: the storage keys, the six-row order, the defaults, and the Referer row are untouched.
...
**Independent Test**: Open the plugin preferences page after upgrade and confirm the first row reads "CBRD Link Endpoint" (not "CBRD API URL"/"CBRD API Endpoint") and the third row reads "CBRD Link timeout (ms)" (not "CBRD timeout (ms)"), while the storage keys and six-row order are unchanged.

**Acceptance Scenarios**:

1. **Given** the editor opens the plugin preferences page, **When** the first row is rendered, **Then** its label reads "CBRD Link Endpoint" in every shipped language (en_US: "CBRD Link Endpoint:", zh_CN/zh_TW: "CBRD 連結端點：").
2. **Given** the editor opens the plugin preferences page, **When** the third row is rendered, **Then** its label reads "CBRD Link timeout (ms)" in every shipped language (en_US: "CBRD Link timeout (ms):", zh_CN/zh_TW: "CBRD 連結逾時 (毫秒)：").
3. **Given** the two label renames are applied, **When** the editor inspects the page, **Then** the six-row order is unchanged (CBRD Link endpoint, Referer header, Link timeout, then CBRD Parse endpoint URL, token, Parse timeout) and the storage keys and defaults are unchanged (FR-023, NFR-005).
4. **Given** the two label renames are applied, **When** Ref-to-Link and AI Markup run, **Then** both actions behave exactly as before — the change is display-only (FR-017).
...
- The Ref-to-Link rows still read "CBRD API URL"/"CBRD timeout (ms)" after the CBRD Parse surface was added → US5 scenarios 1-2, FR-023. The label renames ("CBRD Link Endpoint", "CBRD Link timeout (ms)") are display-only; storage keys, defaults, and the six-row order are unchanged.
...
- **FR-023**: The plugin MUST label the Ref-to-Link lookup endpoint preference "CBRD Link Endpoint" (en_US) / "CBRD 連結端點" (zh_CN/zh_TW) and its timeout preference "CBRD Link timeout (ms)" (en_US) / "CBRD 連結逾時 (毫秒)" (zh_CN/zh_TW) in every shipped language, replacing the pre-rename "CBRD API Endpoint"/"CBRD API URL" and "CBRD timeout (ms)" texts. This rename MUST be display-only: the storage keys (`cbrd.api.url`, `cbrd.referer.header`, `cbrd.timeout`), the six-row preference-page order, the defaults, and the Referer row MUST NOT change, and Ref-to-Link behavior MUST remain unchanged (FR-017, NFR-005).
