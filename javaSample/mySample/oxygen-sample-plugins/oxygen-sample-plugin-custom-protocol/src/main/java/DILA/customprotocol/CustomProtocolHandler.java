...
public class CustomProtocolHandler extends URLStreamHandler {
  ...
  public static File getCanonicalFileFromFileUrl(URL url) {
    ...
    try {
      ...
      String protocol = url.getProtocol().replace("cproto", "file");
      String host = url.getHost();
      String theFile = url.getFile();
      URI newURI = new URL(protocol, host, theFile).toURI();
      file = new File(newURI);
    } catch (URISyntaxException | MalformedURLException e) {
      String uri = url.toExternalForm().replace("cproto:", "file:");
      file = new File(uri);
    }
